...
class TestPresetConfigurations(unittest.TestCase):
    ...
    def test_all_presets_generate(self):
        ...
        for preset_name, config in ConfigurationManager.PRESETS.items():
            with self.subTest(preset=preset_name):
                ...
                try:
                    generator.regen()
                    self.assertTrue(True, f"Preset '{preset_name}' generated successfully")
    ...
    def test_export_import_cycle(self):
        ...
        for preset_name, config in ConfigurationManager.PRESETS.items():
            with self.subTest(preset=preset_name):
                # Export
                export_path = os.path.join(self.test_dir, f"test_config_{preset_name}.json")
                self.assertTrue(
                ...
                for key, value in config.items():
                    self.assertEqual(imported.get(key), value,
...
class TestGeometricValidation(unittest.TestCase):
    ...
    def test_slot_width_constraint(self):
        ...
        radius = config['concentricPolygonRadius']
        max_slot = 2 * radius * np.tan(np.pi / numSides) - 2
    ...
    def test_tactor_magnet_clearance(self):
        ...
        config['tactorRadius'] = 15
        config['magnetRingRadius'] = 18
        config['magnetRadius'] = 5
        
        result = self.validator.validate_complete(config)
        self.assertFalse(result.is_valid, "Tactor-magnet interference not detected")
        self.assertIn('tactorRadius', result.affected_parameters)
    ...
    def test_mount_radius_constraint(self):
        """Test mount radius validation"""
        config = self.base_config.copy()
        
        # Set mount radius too large
        config['mountRadius'] = config['magnetRingRadius'] - config['magnetRadius']
        
        result = self.validator.validate_complete(config)
        self.assertFalse(result.is_valid, "Mount radius constraint not enforced")
        self.assertIn('mountRadius', result.affected_parameters)
    ...
    def test_numSides_variations(self):
        ...
        for num_sides in range(3, 9):
            with self.subTest(numSides=num_sides):
                ...
                radius = config['concentricPolygonRadius']
                max_slot = 2 * radius * np.tan(np.pi / num_sides) - 4
...
class TestParameterInteractions(unittest.TestCase):
    ...
    def test_parameter_dependencies(self):
        ...
        test_cases = [
            ...
            ('concentricPolygonRadius', 20, True),  # Should affect slot width validation
            ('tactorRadius', 15, True),  # Should affect magnet clearance
            ...
            with self.subTest(parameter=param):
                ...
                if should_affect:
                    ...
                    self.assertTrue(has_issues, 
                        f"Parameter {param} change should affect validation")
    ...
    def test_suggestion_generation(self):
        ...
        config['concentricPolygonRadius'] = 25  # Too small
        
        result = self.validator.validate_complete(config)
        
        self.assertFalse(result.is_valid)
        self.assertTrue(len(result.suggestions) > 0, "No suggestions generated for invalid config")
