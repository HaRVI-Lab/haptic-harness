...
class TestReporter:
    ...
    def _test_validation_engine(self):
        ...
        test_cases = [
            ...
            {
                "name": "tactor_too_large",
                "config": {
                    "tactorDiameter": 25 * 2,
                    "concentricPolygonDiameter": 20 * 2,
                },
                "should_fail": True,
            },
            {
    ...
    def _test_parameter_ranges(self):
        ...
        for param_name, param_def in ConfigurationManager.PARAMETERS.items():
            ...
            results[param_name] = {
                "min_in_range": param_def.min_value
                <= param_def.max_value,  # Basic sanity check
                "max_in_range": param_def.max_value
                >= param_def.min_value,  # Basic sanity check
    ...
    def generate_detailed_report(self):
        ...
        for preset, result in preset_results.items():
            ...
            print(
                f"   {preset}: {status} ({result['errors']} errors, {result['warnings']} warnings)"
            )
        ...
        if failed_params:
            ...
            print(
                f"   ✓ PASS: All {len(range_results)} parameters have correct range validation"
            )
